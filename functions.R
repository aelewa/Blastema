...
cv2.var.genes <- function(R,ERCC,plot=FALSE,cutP=0.1,main="CV2 vs mean",quant.fit=0.8,genes=NULL,retP=FALSE){
    ...
    # - retP - T/F - wheter you want the function to return the p-values for the test
   ...
   # remove all genes with sum<1
    ...
    if (retP) {
       return(list(significant=idx.sig, pvalues=p))
    }else {
       return(idx.sig)
    }
}
...
plot.color.bar<-function(col=col.log,crange=crange.log,main="log2(rpkm+1)",horiz=FALSE){
  barplot(rep(1,length(col)),col=col,border=NA,main=main,space=0,axes=F,cex.main=0.6,horiz=horiz)
