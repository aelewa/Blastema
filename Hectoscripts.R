...
# Colors for population A (Kiloscripts) and population B (Hectoscripts) as well as ribogene and transposable element expression
...
# Normalize using kept genes and passed cells. (N.B. basic normalization is sufficient for this analysis)
df_aAn <- sweep(df_aA2[keep,passed], 2, colSums(df_aA2[keep,passed]), FUN="/") *1000000

# Define two populations
popA <- colnames(df_aA2[keep,nD.genes >= 2000 & nD.ercc >=10]) # Kiloscripts (mean=4575 detected genes, sd=1133, n=363 cells)
popB <- colnames(df_aA2[keep,nD.genes < 2000  & nD.ercc >=10]) # Hectoscripts (mean=541 detected genes, sd=418, n=376 cells)
...
# Number of detected genes vs ERCC identifying Hectoscript cells
par(mar=c(4,4,3,20))
...
## Legend: A plot of sequenced spike-in controls (ERCC) and the number of detected genes in each cell reveals two populations. 
##         Kiloscripts express on the order of 10^3 distinct protein coding genes while Hectoscript express on the order of 10^2

# PC1 (all cells) colored by popA vs popB
...
## Legend: Plot demonstrating that the number of detected genes is the primary feature distinguishing the sequenced cells per principal component analysis. 
##         Cells are arranged along the x-axis according to their position in the first principal component
##         and raised along the y-axis according to the number of detected genes. 

# Fig MKNK2 is a marker for popB
...
plot(PC_aAn$x[,'PC1'],as.numeric(df_aAn['MKNK2',]),
     ...
     pch=20,main='',cex.axis=0.7,ylab='MKNK2 expression CPT',xlab='PC1')
...
## Legend: Among Hectoscript cells, MKNK2 is highly expressed, and its expression is dramatically reduced in Kiloscript cells. CPT = Counts Per Thousand

# Top contributors to PC1 (all cells)
...
## Legend: Bar plot showing the top contributors to the positive axis of PC1 (including MKNK2) and thereby markers of Hectoscript cells. 


# MKNK2 is most expressed gene in Hectoscripts
...
## Legend: Bar plot demonstrating that MKNK2 has the highest expression among Hectoscript markers (mean 71,725/cell standard dev. 57,158/cell). 
##         Outliers removed from plot. CPM = Counts Per Million

# Rise of ribogenes, fall of transposable elements
...
abline(v=0,lty=3)
## Legend: Ribogene expression increases, while transposable element expression decreases along the first principal component, 
##         defining the difference between Hectoscripts and Kiloscripts. 
##         Cells were plotted twice, once based on their natural log of total ribogene CPM, and once based on their natural log of total TE CPM. 
##         A spline was computed for each group (degrees of freedom = 4) and overlaid upon the points in the same color. CPM = Counts Per Million.
